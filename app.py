...
<style>
  ...
  :root {
    ...
    --text2: #8b90a8; --success: #4fca7a; --danger: #f74f6a;
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .record-status { font-size:14px; color:var(--text2); text-align:center; }
  .record-status.active { color:var(--danger); font-weight:500; }
  .timer { font-family:'DM Serif Display',serif; font-size:28px; letter-spacing:2px; display:none; }
  .timer.visible { display:block; }
  .btn { width:100%; padding:15px; border-radius:12px; border:none; font-family:'DM Sans',sans-serif; font-size:15px; font-weight:600; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; }
  ...
  .progress-step { display:flex; align-items:center; gap:12px; padding:12px 14px; background:var(--surface2); border-radius:10px; font-size:14px; opacity:0.4; }
  ...
  .download-btn { background:linear-gradient(135deg,var(--success),#3ab868); color:white; }
...
<div class="header">
  <div class="logo">🩺</div>
  <div class="header-text"><h1>MediScribe</h1><p>Automaattinen potilaskertomus</p></div>
</div>
<div class="container">
  <div class="card">
  ...
  <div class="result-section" id="resultSection">
    ...
    <button class="btn download-btn" onclick="downloadPDF()">📥 Lataa potilaskertomus PDF</button>
...
function updateTimer(){seconds++;const m=String(Math.floor(seconds/60)).padStart(2,'0'),s=String(seconds%60).padStart(2,'0');document.getElementById('timer').textContent=`${m}:${s}`;}
async function toggleRecording(){
  if(!isRecording){
    ...
    }catch(e){showError('Mikrofoni ei ole käytettävissä.');}
  }else{
    mediaRecorder.stop();mediaRecorder.stream.getTracks().forEach(t=>t.stop());
    isRecording=false;clearInterval(timerInterval);
    document.getElementById('recordBtn').classList.remove('recording');
    document.getElementById('recordBtn').textContent='🎙️';
    document.getElementById('recordStatus').textContent='Äänitys valmis';
...
async function generate(){
  ...
  try{
    ...
    const blob=await resp2.blob();pdfData=URL.createObjectURL(blob);
    setStep(3,'done');
...
function downloadPDF(){if(!pdfData)return;const a=document.createElement('a');a.href=pdfData;const name=document.getElementById('patientName').value||'potilas';a.download=`potilaskertomus_${name}_${new Date().toISOString().split('T')[0]}.pdf`;a.click();}
function reset(){audioBlob=null;pdfData=null;seconds=0;document.getElementById('timer').textContent='00:00';document.getElementById('timer').classList.remove('visible');document.getElementById('recordStatus').textContent='Paina nappia aloittaaksesi äänitys';document.getElementById('recordStatus').classList.remove('active');document.getElementById('generateBtn').disabled=true;document.getElementById('generateBtn').style.display='flex';document.getElementById('resultSection').classList.remove('visible');document.getElementById('progressSection').classList.remove('visible');hideError();}
...
def generate_pdf():
    ...
    try:
        ...
        footer_text = f"Luotu automaattisesti MediScribe-järjestelmällä • {now}" if language == "fi" else f"Generated automatically by MediScribe • {now}"
